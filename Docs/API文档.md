...
> 场景文件为扁平 DTO（GUID 父子引用 + 组件类型全名 + 公开可写属性值）；纹理存 **GUID + 相对路径**（提供 `IAssetGuidResolver` 时以 GUID 主引用，改名/移动后经 GUID 重连；无解析器则退化为纯路径）。组件经反射重建（`AssemblyQualifiedName` + 公开可写属性），统一覆盖内置与自定义组件；`Canvas` UI 元素树同样经反射序列化（`UiElementData`：类型全名 + 公开可写属性 + 子元素递归，`IsAutoGenerated` 标记的子元素如 Button 标签被跳过）；`SpriteAnimator` 片段暂不覆盖。
...
| 子元素 | `Children` / `AddChild(child)` / `AddChild<T>(child)` / `InsertChild(int, child)` / `RemoveChild` / `RemoveAllChildren` | 子元素树（`InsertChild` 按索引插入，供排序 / 撤销恢复位置） |
