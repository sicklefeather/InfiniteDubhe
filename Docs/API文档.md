...
| `InfiniteDubhe.Engine` | 引擎运行时：`GameHost` 主循环 | Core + Platform + Rendering + Scene + Input + Resources + Audio + UI |
...
`InfiniteDubhe.Platform.Windows` 提供现成实现：`new WindowsPlatformBootstrap()`（对应 `IWindow`/`IGraphicsContext`/`IInputSource`/`IFileSystem`/`IClock` 的 Windows 版）；`WindowsWindow` 额外暴露 `Hwnd`（原生窗口句柄，供编辑器模态对话框等使用）。
...
| `SetSiblingIndex(int)` | 在同级内移动到指定索引（供编辑器拖拽排序 / 撤销恢复位置） |
...
public sealed class SceneSerializer
{
    ...
    public Scene Deserialize(string json, string? name = null);     // JSON → SceneFile → 活对象 + 引用重连（场景名默认 "Scene"）
    public string SerializeSubtree(GameObject root);               // 单对象子树 → JSON（供编辑器删除对象撤销快照）
    public GameObject DeserializeSubtree(string json, Scene scene, GameObject? parent); // 还原子树挂到指定父下，返回子树根
}
...
public sealed class SceneLoader
{
    ...
    public Scene LoadScene(string path, string? name = null);  // name 缺省取 NameFromPath(path)；固定场景（如全局场景）可显式指定
    public static string NameFromPath(string path);            // 剥掉 .json 与约定的 .scene 后缀（global.scene.json → global；空名回退 "Scene"）
...
> 场景文件为扁平 DTO（GUID 父子引用 + 组件类型全名 + 公开可写属性值），**场景名不入 JSON**：加载时以文件名命名（`NameFromPath`），重命名场景 = 重命名文件；`Scene.Name` 可写，供编辑器保存后同步。纹理存 **GUID + 相对路径**（提供 `IAssetGuidResolver` 时以 GUID 主引用，改名/移动后经 GUID 重连；无解析器则退化为纯路径）。组件经反射重建（`AssemblyQualifiedName` + 公开可写属性），统一覆盖内置与自定义组件；`Canvas` UI 元素树同样经反射序列化（`UiElementData`：类型全名 + 公开可写属性 + 子元素递归，`IsAutoGenerated` 标记的子元素如 Button 标签被跳过）；`SpriteAnimator` 片段暂不覆盖。
...
public readonly struct RaycastHit2D { public Vector2 Point { get; } public Vector2 Normal { get; } public float Fraction { get; } public Collider2D Collider { get; } }
public readonly struct Collision2D { public Collider2D Other { get; } public Rigidbody2D? OtherRigidbody { get; } }
```
...
| 子元素 | `Parent` / `Children` / `AddChild(child)` / `AddChild<T>(child)` / `InsertChild(int, child)` / `RemoveChild` / `RemoveAllChildren` | 子元素树（`InsertChild` 按索引插入，供排序 / 撤销恢复位置） |
...
> **`DropdownOption`（下拉框候选项）**：`Text`（显示文本）+ `Value`（逻辑值）；`Value` 未设置（null/空）时有效值 `EffectiveValue` 回落到 `Text`。支持从 `string` 隐式转换（纯文本选项），经场景序列化自动往返；`Dropdown.SelectedItem` 返回选中项 `Text`、`SelectedValue` 返回有效值。
