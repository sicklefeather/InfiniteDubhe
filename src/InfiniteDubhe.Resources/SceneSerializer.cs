...
public sealed class SceneSerializer
{
    ...
    private List<PropertyValue> ToProperties(Component component)
    {
        var result = new List<PropertyValue>();
        foreach (var prop in EnumerateProperties(component.GetType()))
        {
    ...
    private void RestoreComponent(GameObject go, ComponentData data)
    {
        ...
        foreach (var pv in data.Properties)
        {
            var prop = type.GetProperty(pv.Name, BindingFlags.Public | BindingFlags.Instance)
                ?? throw new InvalidOperationException($"组件 {data.TypeName} 上不存在属性 \"{pv.Name}\"。");
            ...
            {
                prop.SetValue(component, pv.Value.Deserialize(prop.PropertyType, _options));
            }
        }
    }
